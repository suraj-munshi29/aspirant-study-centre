    ...
    <>
      ...
      <motion.div
        ...
        className="fixed inset-0 w-full min-h-screen sm:h-[90vh] md:h-[600px] backdrop-blur-lg 
                  rounded-none sm:rounded-lg md:rounded-xl overflow-hidden z-50 flex items-center justify-center bg-black/60"
      >
        {/* Main Content */}
        <div className="h-full w-full flex flex-col justify-center items-center">
          <div className="flex-1 w-full p-2 sm:p-3 md:p-4 flex items-center justify-center">
            <AnimatePresence mode="wait">
              <motion.div
                key={selectedItem.id}
                className="relative w-full aspect-[16/9] max-w-[95%] sm:max-w-[85%] md:max-w-3xl 
                         h-auto max-h-[70vh] rounded-lg overflow-hidden shadow-2xl border border-white/20"
              ...
              >
                <MediaItem
                  item={selectedItem}
                  className="w-full h-full object-contain bg-gray-900/40"
                  onClick={onClose}
                />
                <div
        ...
        <motion.button
          className="absolute top-3 sm:top-4 right-3 sm:right-4 
                    p-2 rounded-full bg-white/20 text-white hover:bg-white/40 
                    text-xs sm:text-sm backdrop-blur-md cursor-pointer border border-white/20 z-50"
          onClick={onClose}
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({
  ...
  return (
    <div className={`w-full max-w-4xl mx-auto px-2 py-4 overflow-visible ${className}`}>
      {(title || description) && (
      ...
      <AnimatePresence mode="wait">
        {selectedItem ? (
          ...
          <motion.div
            className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-5 auto-rows-[100px] sm:auto-rows-[110px] overflow-visible"
            initial="hidden"
            animate="visible"
            exit="hidden"
            variants={{
          ...
          >
            {items.map((item, index) => (
              <motion.div
                ...
                className={`relative overflow-visible rounded-2xl cursor-move group ${item.span}`}
              ...
              >
                {/* Outer glowing aura — matches about section logo */}
                <div className="absolute -inset-3 bg-gradient-to-tr from-primary/30 via-secondary/25 to-primary/40 rounded-3xl blur-2xl animate-pulse opacity-0 group-hover:opacity-90 group-hover:scale-110 transition-all duration-700 pointer-events-none" />

                {/* Card with glow shadow — matches about section logo */}
                <div className="relative w-full h-full overflow-hidden rounded-2xl border border-primary/30 shadow-[0_0_15px_rgba(2,132,199,0.15)] group-hover:shadow-[0_0_50px_rgba(2,132,199,0.3)] dark:shadow-[0_0_20px_rgba(56,189,248,0.1)] dark:group-hover:shadow-[0_0_60px_rgba(56,189,248,0.25)] transition-all duration-500">
                  <MediaItem
                    item={item}
                    className="absolute inset-0 w-full h-full object-cover group-hover:drop-shadow-[0_0_20px_rgba(2,132,199,0.4)] dark:group-hover:drop-shadow-[0_0_25px_rgba(56,189,248,0.5)] transition-all duration-500"
                    onClick={() => !isDragging && setSelectedItem(item)}
                  />
                </div>
              </motion.div>
